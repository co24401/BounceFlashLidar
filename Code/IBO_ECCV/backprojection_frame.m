function voxel_out = backprojection_frame(voxel, illumination_point, detection_point_list)
    %input
    ...
    voxel_out = voxel;
    volume = zeros(size(voxel.volume));
    ...
    for ii = 1:size(detection_point_list,1) %for each line
        ...
        volume = volume + line_detection(volume, line_origin, line_direction, grid3D); 
    end
    %add weight per frame, not per line.
    volume(volume>1) = 1;
